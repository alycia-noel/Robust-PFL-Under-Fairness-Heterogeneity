...
m = "neural-net-two-dp"
seed_everything(0)
data_train_1, data_test_1, data_train_2, data_test_2, data_train_3, data_test_3, features_1, features_2, features_3, decision_1, decision_2, decision_3, d_test_1, d_test_2, d_test_3, encoders = get_data()
...
all_acc_3, all_f_acc_3, all_m_acc_3 = [], [], []
all_test_error_3, all_F_ERR_3, all_M_ERR_3 = [], [], []
all_tp_3, all_tn_3, all_fp_3, all_fn_3, all_f1_3 = [], [], [], [], []
all_F_TP_3, all_F_FP_3, all_F_TN_3, all_F_FN_3, all_F_F1_3 = [], [], [], [], []
all_M_TP_3, all_M_FP_3, all_M_TN_3, all_M_FN_3, all_M_F1_3 = [], [], [], [], []
all_EOD_3, all_SPD_3, all_AOD_3 = [], [], []
all_times_3, all_roc_3 = [], []
...
for i in range(1):
    ...
    for c in range(clients):
        ...
        test_loader = DataLoader(data_test, shuffle = True, batch_size= 128)
        ...
        for epoch in range(epochs):
            ...
            if c == 0:
                ...
                res = (
                    pd.DataFrame(columns=features_3, index=d_test_3.index)
                        .add_suffix('_partial')
                        .join(d_test_3)
                        .assign(prediction=predictions)
                        .assign(round=epoch)
        ...
        if c == 0:
            ...
            all_acc_3.append(test_acc[len(test_acc) - 1])
            all_f_acc_3.append(F_ACC[len(F_ACC) - 1])
            all_m_acc_3.append(M_ACC[len(M_ACC) - 1])
            all_test_error_3.append(test_error[len(test_error) - 1])
            all_F_ERR_3.append(F_ERR[len(F_ERR) - 1])
            all_M_ERR_3.append(M_ERR[len(M_ERR) - 1])
            all_tp_3.append(tp[len(tp) - 1])
            all_tn_3.append(tn[len(tn) - 1])
            all_fp_3.append(fp[len(fp) - 1])
            all_fn_3.append(fn[len(fn) - 1])
            all_f1_3.append(f1[len(f1) - 1])
            all_F_TP_3.append(F_TP[len(F_TP) - 1])
            all_F_FP_3.append(F_FP[len(F_FP) - 1])
            all_F_TN_3.append(F_TN[len(F_TN) - 1])
            all_F_FN_3.append(F_FN[len(F_FN) - 1])
            all_F_F1_3.append(F_F1[len(F_F1) - 1])
            all_M_TP_3.append(M_TP[len(M_TP) - 1])
            all_M_FP_3.append(M_FP[len(M_FP) - 1])
            all_M_TN_3.append(M_TN[len(M_TN) - 1])
            all_M_FN_3.append(M_FN[len(M_FN) - 1])
            all_M_F1_3.append(M_F1[len(M_F1) - 1])
            all_EOD_3.append(EOD[len(EOD) - 1])
            all_SPD_3.append(SPD[len(SPD) - 1])
            all_AOD_3.append(AOD[len(AOD) - 1])
            end = time.time()
            elapsed = end - start
            times.append(elapsed)
            total_time = sum(times)
            all_times_3.append(total_time)
        ...
        if c == 0:
            ...
            all_roc_3.append(plot_roc_curves(results, 'prediction', 'two_year_recid', size=(7, 5), fname='./results/roc.png'))
...
print('*       all       *')
print('*******************')
print('Test Accuracy: {0:1.3f}'.format(np.mean(all_acc_3)))
print('Test Error: {0:1.3f}'.format(np.mean(all_test_error_3)))
print(
    'TP: {0:1.1f};\tFP: {1:1.1f};\tTN: {2:1.1f};\tFN {3:1.1f}'.format(np.mean(all_tp_3), np.mean(all_fp_3), np.mean(all_tn_3),
                                                                      np.mean(all_fn_3)))
print('EOD: {0:1.4f}'.format(np.mean(all_EOD_3)))
print('SPD: {0:1.4f}'.format(np.mean(all_SPD_3)))
print('AOD: {0:1.4f}'.format(np.mean(all_AOD_3)))
print('F1: {0:1.3f}'.format(np.mean(all_f1_3)))
print("")
print('**********************')
print('*       Female       *')
print('**********************')
print('Test Accuracy: {0:1.3f}'.format(np.mean(all_f_acc_3)))
print('Test Error: {0:1.3f}'.format(np.mean(all_F_ERR_3)))
print('TP: {0:1.1f};\tFP: {1:1.1f};\tTN: {2:1.1f};\tFN {3:1.1f}'.format(np.mean(all_F_TP_3), np.mean(all_F_FP_3),
                                                                        np.mean(all_F_TN_3), np.mean(all_F_FN_3)))
print('F1: {0:1.3f}'.format(np.mean(all_F_F1_3)))
print("")
print('********************')
print('*       Male       *')
print('********************')
print('Test Accuracy: {0:1.3f}'.format(np.mean(all_m_acc_3)))
print('Test Error: {0:1.3f}'.format(np.mean(all_M_ERR_3)))
print('TP: {0:1.1f};\tFP: {1:1.1f};\tTN: {2:1.1f};\tFN {3:1.1f}'.format(np.mean(all_M_TP_3), np.mean(all_M_FP_3),
                                                                        np.mean(all_M_TN_3), np.mean(all_M_FN_3)))
print('F1: {0:1.3f}'.format(np.mean(all_M_F1_3)))

print('Train Time: {0:1.2f}'.format(np.mean(all_times_3)))
print('AUROC: {0:1.2f}'.format(np.mean(all_roc_3)))
