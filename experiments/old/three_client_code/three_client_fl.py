...
clients = 2
c1_lr = [.05, .03, .01, .001, .003, .005, .008, .0001, .0003, .0005, .00001, .00003, .00005]
c2_lr = [.05, .03, .01, .001, .003, .005, .008, .0001, .0003, .0005, .00001, .00003, .00005]
o_lr = [.001, .003, .005, .008, .0001, .0003, .0005, .00001, .00003, .00005]
num_hidden = [1, 2, 3]

for k, num in enumerate(num_hidden):
        ...
        for i, c1 in enumerate(c1_lr):
            for j, c2 in enumerate(c2_lr):
                ...
                if m == "log-reg-two-fl":
                    ...
                    hnet = LR_HyperNet(vector_size=10, hidden_dim=100, num_hidden=num)  # 100
                    c1_l = c1
                    c2_l = c2#.0003#.00028
                    o_l = o#.001
                    step = 5
            ...
            # print('*       all       *')
            # print('*******************')
                print('Num_hidden: {1};\t o_l: {2};\t c1_l: {3};\tc2_l: {4};\t C1 Accuracy: {0:1.3f};\t C2 Accuracy: {5:1.3f}'.format(np.mean(all_acc_1),num, o, c1, c2, np.mean(all_acc_2)))
