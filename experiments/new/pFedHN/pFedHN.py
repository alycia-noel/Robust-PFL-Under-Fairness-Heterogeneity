...
def eval_model(nodes, num_nodes, hnet, model, num_features, loss, device, fair, constraint, alpha, confusion, which_position):
    curr_results, pred, true, f1, f1_f, f1_m, a, f_a, m_a, aod, eod, spd = evaluate(nodes, num_nodes, hnet, model, num_features, loss, device, fair, constraint, alpha, which_position)
...
@torch.no_grad()
def evaluate(nodes, num_nodes, hnet, model,  num_features, loss, device, fair, constraint, alpha, which_position):
    hnet.eval()
...
def train(writer, device, data_name,model_name,classes_per_node,num_nodes,steps,inner_steps,lr,inner_lr,wd,inner_wd, hyper_hid,n_hidden,bs, alpha,fair, which_position):
    ...
    for i in range(1):
        ...
        if fair == 'none':
            combo_params = list(model.parameters())
        else:
            combo_params = list(model.parameters()) + list(constraint.parameters())
        ...
        for step in step_iter:
            ...
            if step % 99 == 0 or step == 1499 or step == 0:
                step_results, avg_loss, avg_acc_all, all_acc, all_loss, f1, f1_f, f1_m, f_a, m_a, aod, eod, spd = eval_model(nodes, num_nodes, hnet, model, num_features, loss, device, confusion=False, fair=fair, constraint=constraint, alpha=alpha, which_position=which_position)
        ...
        step_results, avg_loss, avg_acc_all, all_acc, all_loss, f1, f1_f, f1_m, f_a, m_a, aod, eod, spd = eval_model(nodes,
                                                                                                                 num_nodes,
                                                                                                                 hnet,
                                                                                                                 model,
                                                                                                                 num_features,
        ...
        all_times[i].append(step_iter.format_dict["elapsed"])
...
def main():
    ...
    for i in range(len(batch_sizes)):
        ...
        parser.add_argument("--batch_size", type=int, default=batch_sizes)
        ...
        parser.add_argument("--inner_lr", type=float, default=1e-2, help="learning rate for inner optimizer")
        ...
        parser.add_argument("--alpha", type=int, default=100, help="fairness/accuracy trade-off parameter")
        parser.add_argument("--which_position", type=int, default=5, choices=[5,8], help="where the sensitive attribute is. 5: compas, 8: adult")
