...
def train(writer, device, data_name,model_name,classes_per_node,num_nodes,steps,inner_steps,lr,inner_lr,wd,inner_wd, hyper_hid,n_hidden,bs, alpha,fair, which_position, save_file_name):
    ...
    file.write("\nAlpha: {0}, AVG Acc: {1:.4f}, C1 EOD: {2:.4f}, C2 EOD: {3: .4f}, C3 EOD: {4:.4f}, C4 EOD: {5:.4f}, C1 AOD: {6: .4f}, C2 AOD: {7:.4f}, C3 AOD: {8:.4f}, C4 AOD: {9:.4f}".format(alpha, np.mean(avg_acc[0]), np.mean(all_eod[0]), np.mean(all_eod[1]), np.mean(all_eod[2]), np.mean(all_eod[3]), np.mean(all_aod[0]), np.mean(all_aod[1]), np.mean(all_aod[2]), np.mean(all_aod[3]) ))
    file.close()

def main():
    file = open("/home/ancarey/FairFLHN/experiments/new/grid_search/results/eo-compas.txt", "w")
    file.close()
    ...
    for i, a in enumerate(alpha):
        ...
        parser.add_argument("--data_name", type=str, default="compas", choices=["adult", "compas"],
        ...
        parser.add_argument("--alpha", type=int, default=[a, a], help="fairness/accuracy trade-off parameter")
        ...
        parser.add_argument("--save_file_name", type=str,
                            default="/home/ancarey/FairFLHN/experiments/new/grid_search/results/eo-compas.txt")
