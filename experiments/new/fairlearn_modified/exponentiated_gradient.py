...
class ExponentiatedGradient(BaseEstimator, MetaEstimatorMixin):
    ...
    def fit(self, X, y, **kwargs):
        ...
        X : numpy.ndarray or pandas.DataFrame
            Feature data. No context information, just the data itself
        y : numpy.ndarray, pandas.DataFrame, pandas.Series, or list
            Label vector
        """
        self.lambda_vecs_EG_ = pd.DataFrame()  # lambdas for exponentiated gradient
        self.lambda_vecs_LP_ = pd.DataFrame()  # lambdas for linear programming
        ...
        lagrangian = _Lagrangian(
            ...
            **kwargs, # has the sensitive variable information
        )
        ...
        last_gap = np.PINF
        for t in range(0, self.max_iter): # will run 50 times with different laplacian values
            ...
            Q_EG = Qsum / Qsum.sum()
            result_EG = lagrangian.eval_gap(Q_EG, lambda_EG, self.nu) # does not need edited
