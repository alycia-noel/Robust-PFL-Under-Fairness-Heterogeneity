...
def train(writer, device, data_name,model_name,classes_per_node,num_nodes,steps,inner_steps,lr,inner_lr,wd,inner_wd, hyper_hid,n_hidden,bs, alpha,fair, which_position, context_hidden_size, save_file_name):
    ...
    file.write("\nAlpha: {0}, AVG Acc: {1:.4f}, C1 SPD: {2:.4f}, C2 SPD: {3:.4f}, C3 SPD: {4:.4f}, C4 SPD: {5:.4f}".format(alpha, np.mean(avg_acc[0]), np.mean(all_spd[0]), np.mean(all_spd[1]), np.mean(all_spd[2]), np.mean(all_spd[3])))
    file.close()


def main():
    file = open("/home/ancarey/FairFLHN/experiments/new/grid_search/results/dp.txt", "w")
    file.close()
    ...
    for i, alph in enumerate(alphas):
        ...
        parser.add_argument("--save_file_name", type=str, default="/home/ancarey/FairFLHN/experiments/new/grid_search/results/dp.txt")
