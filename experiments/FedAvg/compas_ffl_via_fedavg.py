...
from utils import seed_everything, plot_roc_curves, get_data_no_dp, get_data_dp, confusion_matrix_no_dp, confusion_matrix_dp, metrics
...
def validation_no_dp(model, test_loader, loss):
    model.eval()
    model = model.to(device)
    predictions = []
    TP, FP, FN, TN = 0, 0, 0, 0
    ...
    with torch.no_grad():
        for i, (x, y) in enumerate(test_loader):
            ...
            TP, FP, FN, TN, f_tp, f_fp, f_tn, f_fn, m_tp, m_fp, m_tn, m_fn = confusion_matrix_no_dp(x, predicted_prediction, labels_pred, TP, FP, FN, TN, f_tp, f_fp, f_tn, f_fn, m_tp, m_fp, m_tn, m_fn)

    running_loss_test /= len(test_loader)
    f1_score_prediction, f1_female, f1_male, accuracy, f_acc, m_acc, error, f_err, m_err, aod, eod, spd = metrics(TP, FP, FN, f_tp, f_fp, f_fn, m_tp, m_fp, m_fn, TN, f_tn, m_tn)
...
def train_dp(model, train_loader, loss, optimizer):
    model.train()
    model = model.to(device)
    correct = 0
    running_loss = 0.0
...
def validation_dp(model, test_loader, loss):
    ...
    with torch.no_grad():
        for i, (x, y, s) in enumerate(test_loader):
            ...
            TP, FP, FN, TN, f_tp, f_fp, f_tn, f_fn, m_tp, m_fp, m_tn, m_fn = confusion_matrix_dp(s, predicted_prediction, labels_pred, TP, FP, FN, TN, f_tp, f_fp, f_tn, f_fn, m_tp, m_fp, m_tn, m_fn)
...
def start_train_end_node_process_print_some(number_of_samples, print_amount):
    for i in range(number_of_samples):
        ...
        for epoch in range(num_epoch):
            train_loss, train_accuracy = train_no_dp(model.double(), train_dl, criterion, optimizer)
            predictions, running_loss_test, f1_score_prediction, f1_female, f1_male, accuracy, f_acc, m_acc, error, f_err, m_err, aod, eod, spd, TP, FP, FN, TN, f_tp, f_fp, f_tn, f_fn, m_tp, m_fp, m_tn, m_fn = validation_no_dp(model.double(), test_dl, criterion)
        ...
        print("client: {}".format(i) + " | epoch: {:3.0f}".format(epoch + 1) + " | train accuracy: {:7.5f}".format(
...
seed_everything(0)
features_all, d_test_all, data_test_all, data_train_1, data_test_1, data_train_2, data_test_2, features_1, features_2, decision_1, decision_2, d_test_1, d_test_2, encoders = get_data_no_dp()
...
for i in range(10):
    ...
    predictions, running_loss_test, f1_score_prediction, f1_female, f1_male, accuracy, f_acc, m_acc, error, f_err, m_err, aod, eod, spd, TP, FP, FN, TN, f_tp, f_fp, f_tn, f_fn, m_tp, m_fp, m_tn, m_fn = validation_no_dp(main_model.double(), test_dl, main_criterion)
...
print(
    'TP: {0:1.1f};\tFP: {1:1.1f};\tTN: {2:1.1f};\tFN {3:1.1f}'.format(all_tp_1[len(all_tp_1) - 1], all_fp_1[len(all_fp_1)-1], all_tn_1[len(all_tn_1) - 1], all_fn_1[len(all_fn_1) - 1]))
...
print('TP: {0:1.1f};\tFP: {1:1.1f};\tTN: {2:1.1f};\tFN {3:1.1f}'.format(all_F_TP_1[len(all_F_TP_1) - 1], all_F_FP_1[len(all_F_FP_1) - 1], all_F_TN_1[len(all_F_TN_1) - 1], all_F_FN_1[len(all_F_FN_1) - 1]))
...
print('TP: {0:1.1f};\tFP: {1:1.1f};\tTN: {2:1.1f};\tFN {3:1.1f}'.format(all_M_TP_1[len(all_M_TP_1) - 1], all_M_FP_1[len(all_M_FP_1) - 1], all_M_TN_1[len(all_M_TN_1) - 1], all_M_FN_1[len(all_M_FN_1) - 1]))
...
print(
    'TP: {0:1.1f};\tFP: {1:1.1f};\tTN: {2:1.1f};\tFN {3:1.1f}'.format(all_tp_2[len(all_tp_2) - 1], all_fp_2[len(all_fp_2)-1], all_tn_2[len(all_tn_2) - 1], all_fn_2[len(all_fn_2) - 1]))
...
print('TP: {0:1.1f};\tFP: {1:1.1f};\tTN: {2:1.1f};\tFN {3:1.1f}'.format(all_F_TP_2[len(all_F_TP_2) - 1], all_F_FP_2[len(all_F_FP_2) - 1], all_F_TN_2[len(all_F_TN_2) - 1], all_F_FN_2[len(all_F_FN_2) - 1]))
...
print('TP: {0:1.1f};\tFP: {1:1.1f};\tTN: {2:1.1f};\tFN {3:1.1f}'.format(all_M_TP_2[len(all_M_TP_2) - 1], all_M_FP_2[len(all_M_FP_2) - 1], all_M_TN_2[len(all_M_TN_2) - 1], all_M_FN_2[len(all_M_FN_2) - 1]))
...
print(
    'TP: {0:1.1f};\tFP: {1:1.1f};\tTN: {2:1.1f};\tFN {3:1.1f}'.format(tp_all[len(tp_all) - 1], fp_all[len(fp_all)-1], tn_all[len(tn_all) - 1], fn_all[len(fn_all) - 1]))
...
print('TP: {0:1.1f};\tFP: {1:1.1f};\tTN: {2:1.1f};\tFN {3:1.1f}'.format(F_TP_all[len(F_TP_all) - 1], F_FP_all[len(F_FP_all) - 1], F_TN_all[len(F_TN_all) - 1], F_FN_all[len(F_FN_all) - 1]))
...
print('TP: {0:1.1f};\tFP: {1:1.1f};\tTN: {2:1.1f};\tFN {3:1.1f}'.format(M_TP_all[len(M_TP_all) - 1], M_FP_all[len(M_FP_all) - 1], M_TN_all[len(M_TN_all) - 1], M_FN_all[len(M_FN_all) - 1]))
