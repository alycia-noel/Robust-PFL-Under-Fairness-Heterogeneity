...
class TabularData_no_dp(Dataset):
...
def get_data_no_dp():
    ...
    cols_all = compas.columns
    features_all = cols_all[:-1], cols_all[-1]

    # d_train_all, d_test_all = train_test_split(compas, test_size=617)
    # data_train_all = TabularData_no_dp(d_train_all[features_all].values, d_train_all[decisions_all].values)
    # data_test_all = TabularData_no_dp(d_test_all[features_all].values, d_test_all[decisions_all].values)

    d_train_1, d_test_1 = train_test_split(client_1, test_size=300)
    data_train_1 = TabularData_no_dp(d_train_1[features_1].values, d_train_1[decision_1].values)
    data_test_1 = TabularData_no_dp(d_test_1[features_1].values, d_test_1[decision_1].values)

    d_train_2, d_test_2 = train_test_split(client_2, test_size=300)
    data_train_2 = TabularData_no_dp(d_train_2[features_2].values, d_train_2[decision_2].values)
    data_test_2 = TabularData_no_dp(d_test_2[features_2].values, d_test_2[decision_2].values)

    all_test = [d_test_1, d_test_2]
    d_test_all = pd.concat(all_test)
    data_test_all = TabularData_no_dp(d_test_all[features_1].values, d_test_all[decision_1].values)

    return  features_all, d_test_all, data_test_all, data_train_1, data_test_1, data_train_2, data_test_2, features_1, features_2, decision_1, decision_2, d_test_1, d_test_2, encoders
...
def get_data_dp():
    ...
    d_train_1, d_test_1 = train_test_split(client_1, test_size=300)
    data_train_1 = TabularData_dp(d_train_1[features_1].values, d_train_1[decision_1].values, d_train_1[sensitive_1].values)
    data_test_1 = TabularData_dp(d_test_1[features_1].values, d_test_1[decision_1].values, d_test_1[sensitive_1].values)

    d_train_2, d_test_2 = train_test_split(client_2, test_size=300)
    data_train_2 = TabularData_dp(d_train_2[features_2].values, d_train_2[decision_2].values, d_train_2[sensitive_2].values)
    data_test_2 = TabularData_dp(d_test_2[features_2].values, d_test_2[decision_2].values, d_test_2[sensitive_2].values)

    cols_all = compas.columns
    features_all = cols_all[:-1], cols_all[-1]
    all_test = [d_test_1, d_test_2]
    d_test_all = pd.concat(all_test)
    data_test_all = TabularData_dp(d_test_all[features_1].values, d_test_all[decision_1].values, d_test_all[sensitive_1].values)

    return data_test_all, data_train_1, data_test_1, data_train_2, data_test_2, features_1, features_2, features_all, decision_1, decision_2, d_test_1, d_test_2, d_test_all, encoders


def confusion_matrix_no_dp(x, predicted_prediction, labels_pred, TP, FP, FN, TN, f_tp, f_fp, f_tn, f_fn, m_tp, m_fp, m_tn, m_fn):
...
def confusion_matrix_dp(s, predicted_prediction, labels_pred, TP, FP, FN, TN, f_tp, f_fp, f_tn, f_fn, m_tp, m_fp, m_tn,
                     m_fn):
