...
train_loader = DataLoader(data_train, shuffle = True, batch_size = 128) #64
test_loader = DataLoader(data_test, shuffle = False, batch_size= 128) #64

optimizer = torch.optim.Adam(model.parameters(), lr = .003, betas=(0.9, 0.999), eps=1e-08, weight_decay=0.0000001, amsgrad=False)
loss = nn.BCEWithLogitsLoss(reduction='mean') #Binary Cross Entropy loss
dp_loss = DemographicParityLoss(sensitive_classes=[0, 1], alpha=15)
...
for epoch in range(epochs):
    ...
    print('Epoch: {0}/{1};\t Loss: {2:1.3f};\tAcc:{3:1.3f};\tTime:{4:1.2f}'.format(epoch + 1, 50,
