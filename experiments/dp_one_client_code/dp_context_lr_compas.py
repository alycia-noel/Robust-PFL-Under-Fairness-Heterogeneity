...
model = LR(input_size=9, vector_size=9)
model = model.double()
...
optimizer = torch.optim.Adam(model.parameters(), lr = .001, betas=(0.9, 0.999), eps=1e-08)
loss = nn.BCEWithLogitsLoss(reduction='mean')   #binary logarithmic loss function
dp_loss = DemographicParityLoss(sensitive_classes=[0, 1], alpha=10) #20
